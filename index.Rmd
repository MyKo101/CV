...
read_csv("data/Contact.csv",col_types=cols()) %>%
  mutate(favicon = paste0("img/",medium,"-logo.png"),
         missing_favicon = !file.exists(favicon),
         favicon = if_else(missing_favicon,
                           paste0("http://www.google.com/s2/favicons?domain=",link),
                           favicon))%>%
...
```{r skills, echo=F}
skills_hl <- c("R","RMarkdown","LaTeX","SPSS","NVivo","Git","GitHub","HTML","Photoshop") %>%
  paste0("<img src=\"img/",.,"-logo.png\" height=20px/> ",.) %>%
  paste0(collapse="&mdash;")
```
